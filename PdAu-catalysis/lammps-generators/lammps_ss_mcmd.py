...
def generate_lammps_script(atoms, pctg, steady_state_steps, timestep):
    ...
    dir_path = f"../inputs/set-9/mcmd/{round(timestep * 1000)}-fs/{atoms}-atoms/Pd{pd_percentage}Au{gold_percentage}"
    os.makedirs(dir_path, exist_ok=True)  # Create the directory if it doesn't exist

    daily_steps = steps_per_day[atoms]
    num_parts = -(-steady_state_steps // daily_steps)

    swap_atoms = np.floor(0.5 * min(gold_atoms, atoms - gold_atoms))
    ...
    for part in range(1, num_parts + 1):
        ...
        if part == 1:
            template_file = 'template_ss_mcmd.txt'
        else:
            template_file = 'template_restart_ss_mcmd.txt'
        ...
        with open(template_file, 'r') as file:
            ...
            # Monte-Carlo Swaps
            template = template.replace("${SWAP_ATOMS}", str(round(swap_atoms)))
            template = template.replace("${SWAP_FREQ}", str(100) if part < num_parts else str(round(100)))
            ...
            """ output_name = f"prod_{atoms}_Pd{pd_percentage}Au{gold_percentage}_ss_mcmd{'_restart' if part > 1 else ''}{'' if part == 1 else f'_{part - 1}'}.in"
